...
export default class AppNewAnalysis extends BaseAuthComponent {
  ...
  private async prepareDroneSelection() {
    ...
    this.droneOptions = this.drones.map(drone => ({ value: drone.id, text: this.$t("drone-with-sn", {droneName: drone.custom_name, droneSerialNumber: drone.serial_number}).toString() }));
